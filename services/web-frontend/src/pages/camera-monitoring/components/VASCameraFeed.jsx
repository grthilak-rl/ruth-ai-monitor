...
const VASCameraFeed = ({ camera, onModelToggle, onFullscreen, isFullscreen, onStartStream, onStopStream, activeStreams }) => {
  ...
  const saveViolation = async (result, modelId, snapshotData) => {
    try {
      // Throttle violations per camera - only save once every 5 seconds to avoid spam
      const now = Date.now();
      const cameraKey = `${camera.id}-${modelId}`;

      if (lastViolationTimeRef.current[cameraKey] &&
          now - lastViolationTimeRef.current[cameraKey] < 5000) {
        console.log('[THROTTLE] Violation save throttled (too soon)');
        return;
      }

      lastViolationTimeRef.current[cameraKey] = now;

      // Map model ID to violation type
      let violationType = 'work_at_height'; // default
      if (modelId === 'fall-detection') {
        violationType = 'fall_detection';
      } else if (modelId === 'work-at-height') {
        violationType = 'work_at_height';
      }

      console.log('[SAVE] Saving violation:', {
        camera: camera.id,
        model: modelId,
        type: violationType,
      ...
      const violationData = {
        ...
        bounding_boxes: result.detections?.map(d => d.bbox) || [],
        snapshot_data: snapshotData
      };

      const response = await fetch(`/api/cameras/${camera.id}/violations`, {
        method: 'POST',
        headers: {
      ...
      if (response.ok) {
        console.log('[SUCCESS] Violation saved successfully');
      } else {
        console.error('[ERROR] Failed to save violation:', response.status, response.statusText);
      }

    } catch (error) {
      console.error('[ERROR] Error saving violation:', error);
    }
  };
  ...
  const processFrameForDetection = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        console.log(`[DETECT] Result from ${modelId}:`, {
        ...
        // Always draw detections (or clear if none)
        drawDetections(result, canvasElement, videoElement);

        // If violation detected, log and save it
        if (result.violation_detected) {
          ...
          // Convert canvas to base64 for snapshot
          const snapshotData = canvas.toDataURL('image/jpeg', 0.8).split(',')[1];
          await saveViolation(result, modelId, snapshotData);
        }
      } else {
        console.error(`[ERROR] Detection request failed: ${response.status}`);
      }
    } catch (err) {
      console.error('[ERROR] Detection error:', err.message);
    } finally {
  ...
  const drawDetections = (result, canvas, video) => {
    ...
    // ALWAYS clear previous drawings first
    ctx.clearRect(0, 0, canvas.width, canvas.height);

    // If no detections or no success, just leave canvas clear
  ...
  useEffect(() => {
    ...
    if (feedMode === 'live' && connectionStatus === 'streaming' && camera?.activeModels?.length > 0) {
      ...
      console.log('[START] Starting frame processing for camera', camera?.id);
      frameProcessingRef.current = setInterval(processFrameForDetection, 500);
    } else {
      // Stop frame processing
      console.log('[STOP] Stopping frame processing. Reason:', {
      ...
      // Clear canvas - ensure it clears even if dimensions are not set yet
      if (canvasRef.current) {
        const canvas = canvasRef.current;
        const ctx = canvas.getContext('2d');

        // If canvas has dimensions, clear it
        if (canvas.width > 0 && canvas.height > 0) {
          ctx.clearRect(0, 0, canvas.width, canvas.height);
        }

        // Also reset canvas size to force a complete clear
        const currentWidth = canvas.width;
        const currentHeight = canvas.height;
        canvas.width = currentWidth;
        canvas.height = currentHeight;

        console.log('[CANVAS] Cleared canvas:', { width: canvas.width, height: canvas.height });
      }
    }
  ...
  }, [feedMode, connectionStatus, camera?.activeModels?.length, camera?.activeModels?.join(','), camera?.id]);
