...
const errorHandler = (err, req, res, next) => {
  ...
  // Axios errors (Auth Service integration)
  if (err.isAxiosError) {
    if (err.response) {
      ...
      error = {
        ...
        status: 503,
        details: 'Auth service did not respond'
      };
    } else {
  ...
  // SMS service errors
  if (err.code === 'ENOTFOUND' && err.hostname?.includes('twilio')) {
    error = {
      message: 'SMS Service Unavailable',
      status: 503,
      details: 'Unable to connect to SMS service'
    };
  }
